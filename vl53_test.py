"""VL53L1X на Windows через CH341 - простое чтение расстояния (мм).

Запуск:  python vl53_test.py
Выход:   Ctrl+C
Требует: CH341T подключён, датчик на SDA/SCL/3.3V/GND
"""
import ctypes
import time
...
# Полная ST default config 0x2D..0x87 (из Adafruit VL53L1X)
INIT_SEQ = bytes([
...
def main():
    ...
    if h in (None, 0, -1, 0xFFFFFFFFFFFFFFFF):
        print("CH341 не открылся. Проверь подключение модуля.")
        return
    dll.CH341SetStream(0, 1)

    # ID
    if read8(0x010F) != 0xEA:
        print("VL53L1X не обнаружен.")
        dll.CH341CloseDevice(0)
        return

    # Init
    write_block(0x002D, INIT_SEQ)
    write8(0x0087, 0x40)              # start
    t = 0
    while read8(0x0089) != 0x09 and t < 200:
        time.sleep(0.01)
        t += 1
    write8(0x0086, 0x01)              # clear int
    write8(0x0087, 0x00)              # stop
    write8(0x0008, 0x09)
    write8(0x000B, 0x00)

    print("Готово. Ctrl+C для выхода")
    try:
        while True:
            write8(0x0087, 0x40)      # старт одного кадра
            t = 0
            while read8(0x0089) != 0x09 and t < 200:
                time.sleep(0.01)
                t += 1
            dist = read2(0x0096)
            write8(0x0086, 0x01)      # clear int (для следующего кадра)
            print(dist)
            time.sleep(0.05)
